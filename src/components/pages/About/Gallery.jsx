    ...
    <section className="py-16 bg-secondary">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl lg:text-4xl font-bold text-primary">Inside Our Clinic</h2>
          <p className="text-white/80">A glimpse of our environment and care in action.</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {images.map((src, idx) => (
            <img key={idx} src={src} alt={`about-gallery-${idx}`} className="h-36 w-full object-cover rounded-xl" />
          ))}
        </div>
      </div>
    </section>
